import { useEffect, useState } from "react";
import axios from "axios";

const LOGO = "data:image/jpeg;base64,";

export default function AvocatDashboard() {
  const [avocat, setAvocat] = useState(null);
  const [user, setUser] = useState(null);
  const [loading, setLoading] = useState(true);
  const [activeTab, setActiveTab] = useState("overview");
  const [sidebarOpen, setSidebarOpen] = useState(true);

  useEffect(() => {
    const fetchProfile = async () => {
      try {
        const token = localStorage.getItem("authToken") || localStorage.getItem("token");
        const storedUser = localStorage.getItem("user");
        if (storedUser) setUser(JSON.parse(storedUser));

        const res = await axios.get("http://127.0.0.1:8000/api/avocat/profile", {
          headers: { Authorization: `Bearer ${token}` },
        });
        setAvocat(res.data.avocat);
      } catch (error) {
        console.error(error);
      } finally {
        setLoading(false);
      }
    };
    fetchProfile();
  }, []);

  const handleLogout = () => {
    localStorage.removeItem("authToken");
    localStorage.removeItem("token");
    localStorage.removeItem("user");
    window.location.href = "/login";
  };

  const navItems = [
    { id: "overview", icon: "📊", label: "Tableau de bord" },
    { id: "profile",  icon: "👤", label: "Mon Profil" },
    { id: "dossiers", icon: "📁", label: "Dossiers", badge: "0" },
    { id: "clients",  icon: "👥", label: "Clients", badge: "0" },
    { id: "agenda",   icon: "📅", label: "Agenda" },
    { id: "factures", icon: "💰", label: "Factures" },
    { id: "docs",     icon: "📄", label: "Documents" },
  ];

  const stats = [
    { label: "Dossiers actifs",   value: "0", icon: "📁", color: "#2176d4", bg: "#eff6ff" },
    { label: "Clients",           value: "0", icon: "👥", color: "#059669", bg: "#f0fdf4" },
    { label: "Rendez-vous",       value: "0", icon: "📅", color: "#d97706", bg: "#fffbeb" },
    { label: "Factures en attente", value: "0", icon: "💰", color: "#7c3aed", bg: "#f5f3ff" },
  ];

  if (loading) {
    return (
      <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", background: "#f0f4ff", fontFamily: "sans-serif" }}>
        <div style={{ textAlign: "center" }}>
          <img src={LOGO} alt="Sanad Law" style={{ width: 80, height: 80, objectFit: "contain", marginBottom: 16 }} />
          <div style={{ width: 40, height: 40, border: "4px solid #e2e8f0", borderTop: "4px solid #2176d4", borderRadius: "50%", animation: "spin 0.8s linear infinite", margin: "0 auto" }}></div>
          <style>{`@keyframes spin { to { transform: rotate(360deg); } }`}</style>
        </div>
      </div>
    );
  }

  const displayName = avocat ? `Me. ${avocat.prenom} ${avocat.nom}` : (user?.email || "Avocat");
  const initials = avocat ? `${avocat.prenom?.[0] || ""}${avocat.nom?.[0] || ""}`.toUpperCase() : "AV";

  return (
    <>
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=DM+Serif+Display&family=DM+Sans:wght@300;400;500;600&display=swap');
        * { box-sizing: border-box; margin: 0; padding: 0; }
        body { font-family: 'DM Sans', sans-serif; }

        .dash-root { display: flex; min-height: 100vh; background: #f0f4ff; }

        /* ── SIDEBAR ── */
        .sidebar {
          width: 260px; background: linear-gradient(180deg, #0a2463 0%, #1a3a7c 100%);
          display: flex; flex-direction: column;
          position: fixed; top: 0; left: 0; height: 100vh;
          z-index: 100; transition: width 0.3s ease;
          box-shadow: 4px 0 20px rgba(0,0,0,0.15);
        }
        .sidebar.collapsed { width: 70px; }

        .sidebar-header {
          padding: 24px 20px; border-bottom: 1px solid rgba(255,255,255,0.1);
          display: flex; align-items: center; gap: 12px; min-height: 80px;
        }
        .sidebar-logo {
          width: 40px; height: 40px; flex-shrink: 0;
          background: white; border-radius: 10px; padding: 3px;
          display: flex; align-items: center; justify-content: center; overflow: hidden;
        }
        .sidebar-logo img { width: 100%; height: 100%; object-fit: contain; }
        .sidebar-title { overflow: hidden; transition: opacity 0.2s; }
        .sidebar.collapsed .sidebar-title { opacity: 0; width: 0; }
        .sidebar-name { font-family: 'DM Serif Display', serif; color: white; font-size: 16px; letter-spacing: 0.5px; white-space: nowrap; }
        .sidebar-sub { color: rgba(255,255,255,0.5); font-size: 10px; letter-spacing: 1.5px; text-transform: uppercase; white-space: nowrap; }

        .sidebar-toggle {
          position: absolute; right: -12px; top: 28px;
          width: 24px; height: 24px; background: #2176d4;
          border-radius: 50%; border: none; cursor: pointer;
          color: white; font-size: 12px;
          display: flex; align-items: center; justify-content: center;
          box-shadow: 0 2px 8px rgba(0,0,0,0.2);
        }

        .sidebar-user {
          padding: 16px 20px; border-bottom: 1px solid rgba(255,255,255,0.1);
          display: flex; align-items: center; gap: 12px;
        }
        .sidebar-avatar {
          width: 38px; height: 38px; flex-shrink: 0; border-radius: 50%;
          background: linear-gradient(135deg, #2176d4, #60a5fa);
          display: flex; align-items: center; justify-content: center;
          color: white; font-weight: 700; font-size: 14px;
          border: 2px solid rgba(255,255,255,0.2);
          overflow: hidden;
        }
        .sidebar-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .sidebar-user-info { overflow: hidden; transition: opacity 0.2s; }
        .sidebar.collapsed .sidebar-user-info { opacity: 0; width: 0; }
        .sidebar-user-name { color: white; font-size: 13px; font-weight: 600; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; max-width: 160px; }
        .sidebar-user-role { color: rgba(255,255,255,0.5); font-size: 11px; white-space: nowrap; }

        .sidebar-nav { flex: 1; padding: 12px 10px; overflow-y: auto; }
        .nav-section-label {
          color: rgba(255,255,255,0.35); font-size: 10px; font-weight: 700;
          letter-spacing: 1.5px; text-transform: uppercase;
          padding: 8px 10px 6px; white-space: nowrap;
          transition: opacity 0.2s;
        }
        .sidebar.collapsed .nav-section-label { opacity: 0; }

        .nav-item {
          display: flex; align-items: center; gap: 12px;
          padding: 11px 12px; border-radius: 10px; cursor: pointer;
          transition: all 0.15s; color: rgba(255,255,255,0.65);
          font-size: 14px; font-weight: 500; margin-bottom: 2px;
          white-space: nowrap; position: relative;
        }
        .nav-item:hover { background: rgba(255,255,255,0.08); color: white; }
        .nav-item.active { background: rgba(33,118,212,0.35); color: white; }
        .nav-item.active::before { content: ''; position: absolute; left: 0; top: 50%; transform: translateY(-50%); width: 3px; height: 20px; background: #60a5fa; border-radius: 0 3px 3px 0; }
        .nav-icon { font-size: 18px; flex-shrink: 0; }
        .nav-label { transition: opacity 0.2s; }
        .sidebar.collapsed .nav-label { opacity: 0; width: 0; overflow: hidden; }
        .nav-badge {
          margin-left: auto; background: #2176d4; color: white;
          font-size: 11px; font-weight: 700; padding: 2px 7px;
          border-radius: 10px; transition: opacity 0.2s;
        }
        .sidebar.collapsed .nav-badge { opacity: 0; }

        .sidebar-footer {
          padding: 12px 10px; border-top: 1px solid rgba(255,255,255,0.1);
        }
        .logout-btn {
          display: flex; align-items: center; gap: 12px;
          padding: 11px 12px; border-radius: 10px; cursor: pointer;
          transition: all 0.15s; color: rgba(255,100,100,0.8);
          font-size: 14px; font-weight: 500; background: none; border: none;
          width: 100%; white-space: nowrap; font-family: 'DM Sans', sans-serif;
        }
        .logout-btn:hover { background: rgba(239,68,68,0.15); color: #fca5a5; }
        .sidebar.collapsed .logout-label { opacity: 0; width: 0; overflow: hidden; }

        /* ── MAIN ── */
        .main-content {
          margin-left: 260px; flex: 1; transition: margin-left 0.3s ease;
          min-height: 100vh; display: flex; flex-direction: column;
        }
        .main-content.collapsed { margin-left: 70px; }

        /* ── TOPBAR ── */
        .topbar {
          background: white; padding: 0 32px; height: 64px;
          display: flex; align-items: center; justify-content: space-between;
          box-shadow: 0 1px 8px rgba(0,0,0,0.06);
          position: sticky; top: 0; z-index: 50;
        }
        .topbar-left { display: flex; align-items: center; gap: 16px; }
        .topbar-title { font-family: 'DM Serif Display', serif; font-size: 20px; color: #0a2463; }
        .topbar-breadcrumb { font-size: 13px; color: #94a3b8; }
        .topbar-right { display: flex; align-items: center; gap: 16px; }
        .topbar-btn {
          width: 38px; height: 38px; background: #f8fafc; border: 1px solid #e2e8f0;
          border-radius: 10px; display: flex; align-items: center; justify-content: center;
          cursor: pointer; font-size: 16px; transition: all 0.15s;
        }
        .topbar-btn:hover { background: #eff6ff; border-color: #2176d4; }
        .topbar-date { font-size: 13px; color: #64748b; background: #f8fafc; padding: 6px 12px; border-radius: 8px; border: 1px solid #e2e8f0; }

        /* ── PAGE ── */
        .page-body { padding: 28px 32px; flex: 1; }
        .page-header { margin-bottom: 28px; }
        .page-title { font-family: 'DM Serif Display', serif; font-size: 28px; color: #0f172a; margin-bottom: 4px; }
        .page-subtitle { font-size: 14px; color: #64748b; }

        /* ── STATS ── */
        .stats-grid { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; margin-bottom: 28px; }
        @media (max-width: 1100px) { .stats-grid { grid-template-columns: repeat(2, 1fr); } }
        @media (max-width: 600px) { .stats-grid { grid-template-columns: 1fr; } }
        .stat-card {
          background: white; border-radius: 16px; padding: 22px;
          box-shadow: 0 2px 12px rgba(0,0,0,0.05);
          display: flex; align-items: center; gap: 16px;
          transition: transform 0.15s, box-shadow 0.15s;
        }
        .stat-card:hover { transform: translateY(-2px); box-shadow: 0 6px 20px rgba(0,0,0,0.08); }
        .stat-icon { width: 52px; height: 52px; border-radius: 14px; display: flex; align-items: center; justify-content: center; font-size: 24px; flex-shrink: 0; }
        .stat-info { flex: 1; }
        .stat-value { font-size: 28px; font-weight: 700; color: #0f172a; line-height: 1; margin-bottom: 4px; }
        .stat-label { font-size: 13px; color: #64748b; }
        .stat-trend { font-size: 11px; color: #22c55e; font-weight: 600; margin-top: 2px; }

        /* ── GRID ── */
        .content-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 24px; }
        @media (max-width: 900px) { .content-grid { grid-template-columns: 1fr; } }

        /* ── CARDS ── */
        .card { background: white; border-radius: 16px; box-shadow: 0 2px 12px rgba(0,0,0,0.05); overflow: hidden; }
        .card-header { padding: 20px 24px; border-bottom: 1px solid #f1f5f9; display: flex; align-items: center; justify-content: space-between; }
        .card-title { font-size: 15px; font-weight: 700; color: #0f172a; display: flex; align-items: center; gap: 8px; }
        .card-body { padding: 24px; }
        .card-action { font-size: 13px; color: #2176d4; font-weight: 500; cursor: pointer; text-decoration: none; }
        .card-action:hover { text-decoration: underline; }

        /* ── PROFILE CARD ── */
        .profile-hero {
          background: linear-gradient(135deg, #0a2463 0%, #2176d4 100%);
          border-radius: 16px; padding: 28px; margin-bottom: 24px;
          display: flex; align-items: center; gap: 24px;
          box-shadow: 0 4px 20px rgba(33,118,212,0.25);
        }
        .profile-avatar-big {
          width: 90px; height: 90px; border-radius: 50%;
          border: 3px solid rgba(255,255,255,0.3);
          background: rgba(255,255,255,0.15);
          display: flex; align-items: center; justify-content: center;
          color: white; font-size: 28px; font-weight: 700; flex-shrink: 0;
          overflow: hidden;
        }
        .profile-avatar-big img { width: 100%; height: 100%; object-fit: cover; }
        .profile-hero-info { flex: 1; }
        .profile-hero-name { font-family: 'DM Serif Display', serif; font-size: 24px; color: white; margin-bottom: 4px; }
        .profile-hero-email { color: rgba(255,255,255,0.7); font-size: 14px; margin-bottom: 12px; }
        .profile-hero-tags { display: flex; gap: 8px; flex-wrap: wrap; }
        .profile-tag {
          background: rgba(255,255,255,0.15); color: white;
          font-size: 12px; padding: 4px 12px; border-radius: 20px;
          border: 1px solid rgba(255,255,255,0.2);
        }

        /* ── INFO GRID ── */
        .info-grid { display: grid; grid-template-columns: 1fr 1fr; gap: 16px; }
        @media (max-width: 600px) { .info-grid { grid-template-columns: 1fr; } }
        .info-item { display: flex; flex-direction: column; gap: 4px; }
        .info-label { font-size: 11px; font-weight: 700; color: #94a3b8; letter-spacing: 0.5px; text-transform: uppercase; }
        .info-value { font-size: 14px; color: #0f172a; font-weight: 500; }
        .info-value.empty { color: #cbd5e1; font-style: italic; }

        /* ── EMPTY STATE ── */
        .empty-state { text-align: center; padding: 40px 20px; }
        .empty-icon { font-size: 40px; margin-bottom: 12px; opacity: 0.4; }
        .empty-text { color: #94a3b8; font-size: 14px; }

        /* ── ACTIVITY ── */
        .activity-item { display: flex; align-items: flex-start; gap: 14px; padding: 14px 0; border-bottom: 1px solid #f1f5f9; }
        .activity-item:last-child { border-bottom: none; }
        .activity-dot { width: 36px; height: 36px; border-radius: 10px; display: flex; align-items: center; justify-content: center; font-size: 16px; flex-shrink: 0; }
        .activity-info { flex: 1; }
        .activity-title { font-size: 14px; font-weight: 500; color: #0f172a; margin-bottom: 2px; }
        .activity-time { font-size: 12px; color: #94a3b8; }

        /* ── QUICK ACTIONS ── */
        .quick-actions { display: grid; grid-template-columns: 1fr 1fr; gap: 12px; }
        .quick-action {
          padding: 16px; border: 1.5px solid #e2e8f0; border-radius: 12px;
          cursor: pointer; transition: all 0.15s; text-align: center;
          background: #f8fafc;
        }
        .quick-action:hover { border-color: #2176d4; background: #eff6ff; }
        .quick-action-icon { font-size: 24px; margin-bottom: 8px; }
        .quick-action-label { font-size: 13px; font-weight: 600; color: #374151; }

        @keyframes spin { to { transform: rotate(360deg); } }
        @media (max-width: 768px) {
          .sidebar { width: 70px; }
          .sidebar-title, .sidebar-user-info, .nav-label, .nav-badge, .nav-section-label, .logout-label { opacity: 0; width: 0; overflow: hidden; }
          .main-content { margin-left: 70px; }
          .page-body { padding: 20px 16px; }
          .topbar { padding: 0 16px; }
        }
      `}</style>

      <div className="dash-root">
        {/* ── SIDEBAR ── */}
        <aside className={`sidebar ${sidebarOpen ? "" : "collapsed"}`}>
          <button className="sidebar-toggle" onClick={() => setSidebarOpen(!sidebarOpen)}>
            {sidebarOpen ? "◀" : "▶"}
          </button>

          {/* Logo */}
          <div className="sidebar-header">
            <div className="sidebar-logo"><img src={LOGO} alt="Sanad Law" /></div>
            <div className="sidebar-title">
              <div className="sidebar-name">Sanad Law</div>
              <div className="sidebar-sub">Gestion Juridique</div>
            </div>
          </div>

          {/* User info */}
          <div className="sidebar-user">
            <div className="sidebar-avatar">
              {avocat?.photo
                ? <img src={`http://127.0.0.1:8000/storage/${avocat.photo}`} alt="avatar" />
                : initials
              }
            </div>
            <div className="sidebar-user-info">
              <div className="sidebar-user-name">{displayName}</div>
              <div className="sidebar-user-role">Avocat</div>
            </div>
          </div>

          {/* Nav */}
          <nav className="sidebar-nav">
            <div className="nav-section-label">Principal</div>
            {navItems.map((item) => (
              <div
                key={item.id}
                className={`nav-item ${activeTab === item.id ? "active" : ""}`}
                onClick={() => setActiveTab(item.id)}
              >
                <span className="nav-icon">{item.icon}</span>
                <span className="nav-label">{item.label}</span>
                {item.badge && <span className="nav-badge">{item.badge}</span>}
              </div>
            ))}
          </nav>

          {/* Logout */}
          <div className="sidebar-footer">
            <button className="logout-btn" onClick={handleLogout}>
              <span style={{ fontSize: 18 }}>🚪</span>
              <span className="logout-label">Déconnexion</span>
            </button>
          </div>
        </aside>

        {/* ── MAIN ── */}
        <main className={`main-content ${sidebarOpen ? "" : "collapsed"}`}>
          {/* Topbar */}
          <div className="topbar">
            <div className="topbar-left">
              <div className="topbar-title">
                {navItems.find(n => n.id === activeTab)?.icon}{" "}
                {navItems.find(n => n.id === activeTab)?.label}
              </div>
            </div>
            <div className="topbar-right">
              <div className="topbar-date">
                📅 {new Date().toLocaleDateString("fr-FR", { weekday: "long", day: "numeric", month: "long", year: "numeric" })}
              </div>
              <div className="topbar-btn" title="Notifications">🔔</div>
              <div className="topbar-btn" title="Paramètres">⚙️</div>
            </div>
          </div>

          {/* Body */}
          <div className="page-body">

            {/* ── OVERVIEW TAB ── */}
            {activeTab === "overview" && (
              <>
                <div className="page-header">
                  <div className="page-title">Bonjour, {avocat ? `Me. ${avocat.prenom}` : "Avocat"} 👋</div>
                  <div className="page-subtitle">Voici un aperçu de votre activité juridique</div>
                </div>

                {/* Stats */}
                <div className="stats-grid">
                  {stats.map((s, i) => (
                    <div className="stat-card" key={i}>
                      <div className="stat-icon" style={{ background: s.bg }}>
                        {s.icon}
                      </div>
                      <div className="stat-info">
                        <div className="stat-value" style={{ color: s.color }}>{s.value}</div>
                        <div className="stat-label">{s.label}</div>
                      </div>
                    </div>
                  ))}
                </div>

                <div className="content-grid">
                  {/* Activité récente */}
                  <div className="card">
                    <div className="card-header">
                      <div className="card-title">🕐 Activité récente</div>
                      <a className="card-action">Voir tout</a>
                    </div>
                    <div className="card-body">
                      <div className="empty-state">
                        <div className="empty-icon">📋</div>
                        <div className="empty-text">Aucune activité récente</div>
                      </div>
                    </div>
                  </div>

                  {/* Actions rapides */}
                  <div className="card">
                    <div className="card-header">
                      <div className="card-title">⚡ Actions rapides</div>
                    </div>
                    <div className="card-body">
                      <div className="quick-actions">
                        {[
                          { icon: "📁", label: "Nouveau dossier" },
                          { icon: "👤", label: "Nouveau client" },
                          { icon: "📅", label: "Rendez-vous" },
                          { icon: "💰", label: "Nouvelle facture" },
                        ].map((a, i) => (
                          <div className="quick-action" key={i}>
                            <div className="quick-action-icon">{a.icon}</div>
                            <div className="quick-action-label">{a.label}</div>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>
                </div>
              </>
            )}

            {/* ── PROFILE TAB ── */}
            {activeTab === "profile" && (
              <>
                <div className="page-header">
                  <div className="page-title">Mon Profil</div>
                  <div className="page-subtitle">Vos informations professionnelles</div>
                </div>

                {/* Hero */}
                <div className="profile-hero">
                  <div className="profile-avatar-big">
                    {avocat?.photo
                        ? <img src={`http://127.0.0.1:8000/storage/${avocat.photo}`} alt="profile" />
                      : initials
                    }
                  </div>
                  <div className="profile-hero-info">
                    <div className="profile-hero-name">{displayName}</div>
                    <div className="profile-hero-email">{user?.email || ""}</div>
                    <div className="profile-hero-tags">
                      {avocat?.barreau && <span className="profile-tag">⚖️ Barreau de {avocat.barreau}</span>}
                      {avocat?.grade && <span className="profile-tag">🎓 {avocat.grade}</span>}
                      {avocat?.specialite && <span className="profile-tag">🔬 {avocat.specialite}</span>}
                    </div>
                  </div>
                </div>

                {/* Infos grid */}
                <div className="content-grid">
                  <div className="card">
                    <div className="card-header">
                      <div className="card-title">👤 Informations personnelles</div>
                    </div>
                    <div className="card-body">
                      <div className="info-grid">
                        {[
                          { label: "Nom", value: avocat?.nom },
                          { label: "Prénom", value: avocat?.prenom },
                          { label: "Téléphone", value: avocat?.telephone },
                          { label: "Ville", value: avocat?.ville },
                        ].map((item, i) => (
                          <div className="info-item" key={i}>
                            <div className="info-label">{item.label}</div>
                            <div className={`info-value ${!item.value ? "empty" : ""}`}>
                              {item.value || "Non renseigné"}
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>

                  <div className="card">
                    <div className="card-header">
                      <div className="card-title">⚖️ Informations légales</div>
                    </div>
                    <div className="card-body">
                      <div className="info-grid">
                        {[
                          { label: "Numéro d'ordre", value: avocat?.numero_ordre },
                          { label: "Barreau", value: avocat?.barreau },
                          { label: "Grade", value: avocat?.grade },
                          { label: "Date d'inscription", value: avocat?.date_inscription_barreau },
                          { label: "Spécialité", value: avocat?.specialite },
                        ].map((item, i) => (
                          <div className="info-item" key={i}>
                            <div className="info-label">{item.label}</div>
                            <div className={`info-value ${!item.value ? "empty" : ""}`}>
                              {item.value || "Non renseigné"}
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>

                  <div className="card" style={{ gridColumn: "1 / -1" }}>
                    <div className="card-header">
                      <div className="card-title">🏛️ Cabinet</div>
                    </div>
                    <div className="card-body">
                      <div className="info-grid">
                        {[
                          { label: "Nom du cabinet", value: avocat?.nom_cabinet },
                          { label: "Adresse", value: avocat?.adresse_cabinet },
                          { label: "Ville", value: avocat?.ville },
                        ].map((item, i) => (
                          <div className="info-item" key={i}>
                            <div className="info-label">{item.label}</div>
                            <div className={`info-value ${!item.value ? "empty" : ""}`}>
                              {item.value || "Non renseigné"}
                            </div>
                          </div>
                        ))}
                      </div>
                    </div>
                  </div>
                </div>
              </>
            )}

            {/* ── OTHER TABS ── */}
            {!["overview", "profile"].includes(activeTab) && (
              <>
                <div className="page-header">
                  <div className="page-title">
                    {navItems.find(n => n.id === activeTab)?.icon}{" "}
                    {navItems.find(n => n.id === activeTab)?.label}
                  </div>
                  <div className="page-subtitle">Cette section sera disponible prochainement</div>
                </div>
                <div className="card">
                  <div className="card-body">
                    <div className="empty-state" style={{ padding: "60px 20px" }}>
                      <div className="empty-icon" style={{ fontSize: 56, opacity: 0.3 }}>
                        {navItems.find(n => n.id === activeTab)?.icon}
                      </div>
                      <div className="empty-text" style={{ fontSize: 16, marginTop: 12 }}>
                        Module en cours de développement
                      </div>
                      <div className="empty-text" style={{ marginTop: 6 }}>
                        Cette fonctionnalité sera disponible dans la prochaine version
                      </div>
                    </div>
                  </div>
                </div>
              </>
            )}

          </div>
        </main>
      </div>
    </>
  );
}